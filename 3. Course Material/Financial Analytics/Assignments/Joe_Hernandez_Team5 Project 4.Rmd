...
mu.free <-  .0024/252 ## input value of risk-free interest rate
...
When we  short some commodity we open up more funds for the purchase of other stocks, because short a commodity we receive the dollar value of that commodity today and we can turn around and re-invest those dollars for transportation costs or other investments.  
...
2. Portfolio Analytics: Markowitz tangency, Investments, Efficient portfolio mix with risk-free asset, Solver for weight distribution, Market risk, Expected Shortfall, and Value at Risk

### Portfolio Weights

For the working capital accounts:

No constraints - Markowitz Tangency [default] portfolio

* $0 denominated in euros 
* $132,672,225 in nickel
* $57,669,725 in copper
* $59,658,050 in aluminium

The weights in our default Markowitz model show a Nickel heavy allocation of assets to the portfolio, which gives us the optimal allocation to the portfolio, with little consideration to risk. All considering, the default approach would drive us to take an alternate avenue to alleviate portfolio risk.

Our second optimization we begin to add constraints. Here we are taking a minimum variance of prices to aleviate any long term risks and additionally optimize our portfolio. The weights for the Markowitz tangency [min var]  portfolio  ("-") are

By distributing our $250 million euros across the weights for the Markowitz tangency [min variance] we get the following:

* $0 million denominated in euros 
* $21,438,648 in nickel
* $113,727,083 in copper
* $114,834,270 in aluminium

Observing the minimum vairance Markowitz we can view a more diverse portfolio, well spread out unlike the default or 'no short' portfolio weights. Risk level would be considered low for the holding party comparitively to the other two approaches. High allocation to both copper and aluminium.

Finally, we take out any short-term volatility within the market to optimize the portfolio over time. Very similair to the final output of the default Markowitz. The weights for the Markowitz tangency [no short] portfolio ("+") are
...
- The working capital account of $250 million Euro should be allocated as follows: buy $21.4386475 million in nickel, $113.7270825 in copper, and $114.85427 in aluminium.
- We opted to use the Markowitz tangency [min variance] model since we are new to the nickel business. However, over time and as we learn more about this business, the allocation can be adjusted to invest more in nickel.
- Investing more than half of the $250 million Euro in nickel as suggested by the default and no-short models is too large of a risk for the company to take today, particularly in a new business where we perhaps don't have a large distribution network to sell it. Also, we lack the experience and expertise relative to our competitors. A more conservative approach in a commodity market is preferable.
- Additionally, nickel is a more volatile commodity than copper and aluminium. This is reflected in the metals price percent changes graphs and returns distribution graphs, where nickel experiences the most volatility with points appearing the furthest from the mean.
- Lastly, copper and aluminium are negatively correlated (-0.25) and therefore, we would want to have about equal amounts of these commodities in our portfolio to hedge our bets. However, nickel and copper are positively correlated (0.89) and we would want some copper in our portfolio to increase our expected returns, but within some level of tolerable risk.
- For tolerable risk we opted to use 95% quantile. For nickel, the value at risk is 2.73 and expected shortfall is 3.5. For copper, the value at risk is 1.87 and expected shortfall is 2.27. For Aluminium the value at risk is 1.96 and expected shortfall is 2.58. Any future returns that exceeds this threshold will warrant further review of the entire portfolio and perhaps adjustments to it.
...
https://api.zoom.us/recording/play/0KNukn3xXYXzDHmDJm8_BOaPADMC0e1De4qytiE93A5rYvDQvCq56Qn-3a-1N3Pf

* 57:00 Explains function where the weights are distributed by the risk-free rate which in this is a single value. This is done in excel.
* 1:07:00 Explains the duties for the assignment. He actually mentions what and where to put the answers so let's make sure we do that.

https://api.zoom.us/recording/play/VJYuvN3vF3OZqukT8CMCeThyIyx_rQE2XBVJ92dbRg6l3xHJfr_ztdMTgByqTWUI

* 41:00 Coding for Assignment 4 where just prior to 55:00 he goes over weights which is already in the code. The conclusion shows the distribution of the weights.
* 55:00 Rate calculation. I used the current rate of 2.127% over 252 trading days.
* 1:17:30 Pene you asked a question regarding the weight where the answer is to multiply the weight by the investment.
* 1:24:00 Emphasizes what needs completion.
* 1:27:00 References Chapter 11 for rationalizing our answers.
